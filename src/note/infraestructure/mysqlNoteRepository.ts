...
export class MysqlNoteRepository implements NoteRepository {
    ...
    async createNote(uuid: string, user_uuid: string, title: string, description: string, url_file: string, type_file: string, status: boolean): Promise<string | Note | Error | null> {
        try {
    ...
    async getNoteByUser(user_uuid: string): Promise<Note[] | Error | null> {
        try {
            // Selecciona todas las notas del usuario con url_file no vacío o null
            const sql = "SELECT * FROM notes WHERE user_uuid = ? AND (description IS NOT NULL AND description != '')";
            const params: any[] = [user_uuid];
            const [result]: any = await query(sql, params);
