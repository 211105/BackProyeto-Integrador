...
export class MysqlOwnerRepository implements OwnerRepository{
    ...
    async updateOwner(uuid: string, name?: string, surname?: string, second_surname?: string, email?: string, phone_number?: string, img_url?: string): Promise<Owner | null> {
      ...
      // Obtiene la URL de la imagen actual antes de la actualización
      const [existingRows]: any = await query('SELECT img_url FROM owners WHERE uuid = ?', [uuid]);
      ...
      // Elimina el archivo en S3 solo si img_url está presente en la solicitud y hay una imagen existente
      if (img_url && existingImgUrl) {
          await deleteFromS3(existingImgUrl);
      }
      ...
      try {
          ...
          // Retorna el objeto Owner actualizado
